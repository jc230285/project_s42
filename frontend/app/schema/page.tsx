...
function SchemaPageContent() {
  ...
  const handleGetSchemaTable = async () => {
    ...
    try {
      const response = await makeAuthenticatedRequest(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/projects/schema`, {
        method: 'GET',
      });
  ...
  const handleNocoDBSync = async () => {
    ...
    try {
      const response = await makeAuthenticatedRequest(`${process.env.NEXT_PUBLIC_BACKEND_BASE_URL}/nocodb-sync`, {
        method: 'POST',
      });
